...
class ESKF {
   ...
   private:
    ...
    double t12 = 0, t34 = 0, t56 = 0, cnt = 0;
};
...
bool ESKF<S>::Predict(const IMU& imu) {
    ...
    F.template block<3, 3>(6, 9) = -Mat3T::Identity() * dt;                        // theta 对 
    // mean and cov prediction
    // dx_ = F * dx_;  // 这行其实没必要算，dx_在重置之后应该为零，因此这步可以跳过，但F需要参与Cov部分计算，所以保留
    auto t2 = std::chrono::high_resolution_clock::now();
    t12 += std::chrono::duration_cast<std::chrono::duration<double>>(t2 - t1).count() * 1000;

    auto t3 = std::chrono::high_resolution_clock::now();
    // 变量顺序：p, v, theta, bg, ba, grav
    auto delta_p = dx_.template block<3, 1>(0, 0) + dx_.template block<3, 1>(3, 0) * dt;
    auto delta_v = dx_.template block<3, 1>(3, 0) + F.template block<3, 3>(3, 6) * dx_.template block<3, 1>(6, 0)
                + F.template block<3, 3>(3, 12) * dx_.template block<3, 1>(12, 0) + dx_.template block<3, 1>(15, 0) * dt; 
    auto delta_theta  = F.template block<3, 3>(6, 6) * dx_.template block<3, 1>(6, 0) - dx_.template block<3, 1>(9, 0) * dt;
    auto delta_bg = dx_.template block<3, 1>(9, 0);
    auto delta_ba = dx_.template block<3, 1>(12, 0);
    auto delta_g = dx_.template block<3, 1>(15, 0);

    Vec18T dx_new = Vec18T::Zero();
    dx_new.template block<3, 1>(0, 0) = delta_p;
    dx_new.template block<3, 1>(3, 0) = delta_v;
    dx_new.template block<3, 1>(6, 0) = delta_theta;
    dx_new.template block<3, 1>(9, 0) = delta_bg;
    dx_new.template block<3, 1>(12, 0) = delta_ba;
    dx_new.template block<3, 1>(15, 0) = delta_g;
    // dx_ = dx_new;
    auto t4 = std::chrono::high_resolution_clock::now();
    t34 += std::chrono::duration_cast<std::chrono::duration<double>>(t4 - t3).count() * 1000;

    auto t5 = std::chrono::high_resolution_clock::now();
    Vec18T dx_Fs = Vec18T::Zero();
    Eigen::Matrix<S, 9, 18> F_small = Eigen::Matrix<S, 9, 18>::Identity();
    F_small.template block<3, 3>(0, 3) = Mat3T::Identity() * dt;                         // p 对 v
    F_small.template block<3, 3>(3, 6) = -R_.matrix() * SO3::hat(imu.acce_ - ba_) * dt;  // v对theta
    F_small.template block<3, 3>(3, 12) = -R_.matrix() * dt;                             // v 对 ba
    F_small.template block<3, 3>(3, 15) = Mat3T::Identity() * dt;                        // v 对 g
    F_small.template block<3, 3>(6, 6) = SO3::exp(-(imu.gyro_ - bg_) * dt).matrix();     // theta 对 theta
    F_small.template block<3, 3>(6, 9) = -Mat3T::Identity() * dt;                        // theta 对 bg
    dx_Fs.template block<9, 1>(0, 0) = F_small * dx_;
    dx_Fs.template block<3, 1>(9, 0) = dx_.template block<3, 1>(9, 0);
    dx_Fs.template block<3, 1>(12, 0) = dx_.template block<3, 1>(12, 0);
    dx_Fs.template block<3, 1>(15, 0) = dx_.template block<3, 1>(15, 0);
    dx_ = dx_Fs;
    auto t6 = std::chrono::high_resolution_clock::now();
    t56 += std::chrono::duration_cast<std::chrono::duration<double>>(t6 - t5).count() * 1000;

    if (10000 == (++cnt)) {
        LOG(INFO) << "F矩阵-列方程-缩减F矩阵(ms): " << t12 << "--" << t34 << "--" << t56;
        cnt = 0;
        t12 = 0;
        t34 = 0;
        t56 = 0;
...
bool ESKF<S>::ObserveSE3(const SE3& pose, double trans_noise, double ang_noise) {
    ...
    auto delta_p = dx_.template block<3, 1>(0, 0);
    auto delta_v = dx_.template block<3, 1>(3, 0);
    auto delta_R = SO3::exp(dx_.template block<3, 1>(6, 0));
    auto delta_bg = dx_.template block<3, 1>(9, 0);
    auto delta_ba= dx_.template block<3, 1>(12, 0);
    auto delta_g = dx_.template block<3, 1>(15, 0);
    LOG(INFO) << "\ndelta_p\n" << delta_p 
