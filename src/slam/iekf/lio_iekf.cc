...
#include "iekf/lio_iekf.h"
#include "types/g2o_types.h"
...
void LioIEKF::Undistort() {
    ...
    if (options_.save_motion_undistortion_pcd_) {
        sad::SaveCloudToFile("./data/result/before_undist.pcd", *cloud);
    }
    ...
    if (options_.save_motion_undistortion_pcd_) {
        sad::SaveCloudToFile("./data/result/after_undist.pcd", *cloud);
    }
}
