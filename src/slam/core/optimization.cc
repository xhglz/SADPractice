...
void Optimization::Run() {
    ...
    SaveG2O("./data/result/before.g2o");
...
void Optimization::SaveResults() {
    ...
    // 写入文件
    system("rm ./data/result/keyframes.txt");
    std::ofstream fout("./data/result/keyframes.txt");
    for (auto& kfp : keyframes_) {
...
void Optimization::LoadLoopCandidates() {
    std::ifstream fin("./data/result/loops.txt");
    if (!fin) {
        LOG(WARNING) << "cannot load file: ./data/result/loops.txt";
        return;
    }
