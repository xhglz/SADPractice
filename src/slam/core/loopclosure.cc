...
#include "core/loopclosure.h"

#include "lidar/lidar_utils.h"
#include "pointcloud/point_cloud_utils.h"
...
bool LoopClosure::Init() {
    if (!LoadKeyFrames("./data/result/keyframes.txt", keyframes_)) {
...
void LoopClosure::ComputeForCandidate(sad::LoopCandidate& c) {
    ...
    auto build_submap = [this](int given_id, bool build_in_world) -> CloudPtr {
        ...
        for (int idx = -submap_idx_range; idx < submap_idx_range; idx += 4) {
            ...
            pcl::io::loadPCDFile("./data/result/" + std::to_string(id) + ".pcd", *cloud);
    ...
    pcl::io::loadPCDFile("./data/result/" + std::to_string(kf2->id_) + ".pcd", *submap_kf2);
...
void LoopClosure::SaveResults() {
    ...
    std::ofstream fout("./data/result/loops.txt");
    for (const auto& lc : loop_candiates_) {
